	...
	var timer,interval;
	var patternSet=false,patternCorrect=false,drawing=false;
	var canvasid="#mycanvas";
	var changeBtnId="#change";
	var outputId="#msg";
	var flag="bxrYBIEfWB2H943J7UtDscKsM6yp3/o5lykjy5o86+ZPFYRwsBC4bS9o4xXb80/20+Yr";
...
$(document).ready(function(){
	...
	$(canvasid).on('mouseup',function(e){
		drawing=false;
		if(patternCheck()==true && pattern[0] != 0){
			$(outputId).html("Flag: "+rc4(pattern.join(""),atob(flag)));
			patternCorrect=true;
		}else{
			$(outputId).html("Wrong...");
		}
...
function patternCheck(){
	if (pattern[0] != pattern[6] + pattern[4]){
		return false
	}
	if (pattern[1] != pattern[2] + pattern[7]){
		return false
	}
	if (pattern[2] != pattern[8] - pattern[5]){
		return false
	}
	if (pattern[3] != pattern[1] - pattern[5]){
		return false
	}
	if (pattern[4] != pattern[5] + pattern[8]){
		return false
	}
	...
	if (pattern[5] != pattern[7] - pattern[6]){
		return false
	}
	if (pattern[6] != pattern[1] - pattern[8]){
		return false
	}
	if (pattern[7] != pattern[0] - pattern[8]){
		return false
	}
	if (pattern[8] != pattern[3] + pattern[6]){
		return false
	}
	return true;
}
